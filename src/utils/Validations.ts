...
export const validateImageUri = (uri?: string) => uri && validateUrl(uri) && /^.*\.(png|jpg|jpeg|bmp|gif)$/.test(uri);

export const validatePersonName = (name?: string) => name && isNotEmpty(name) && /^([a-zÀ-ƒ]+\s?[a-zÀ-ƒ]+)+$/i.test(name);

export const validateOrganizationName = (organizationName?: string) => organizationName
    && isNotEmpty(organizationName)
...
export const validatePassword = (password?: string, pattern: RegExp = STRONG_PASSWORD_PATTERN) =>
    password && isNotEmpty(password) && pattern.test(password);

export const validateEmail = (email?: string, pattern: RegExp = EMAIL_PATTERN) =>
    email && isNotEmpty(email) && pattern.test(email);

export const validateOtp = (verificationCode: string) => verificationCode
    && isNotSequencedOrRepeated(verificationCode)
    && /^(?=[0-9]).{6}$/.test(verificationCode);

export const isNotSequencedOrRepeated = (string?: string) => string
    && !isEmpty(string)
...
export const isNumber = (value?: string) => value && !isEmpty(value) && !isNaN(+value);
